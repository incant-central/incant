...
async function attemptExternalResolve(name) {
    ...
    try {
        return await loader({ spec: main, input }, { Resource: name, explicit });
    } catch (e) {
        if (e.code === 'DEPRECATED' || explicit) throw e;
        return await resolvers.incant({ spec: main }, { Resource: name, explicit: false });
    }
}
