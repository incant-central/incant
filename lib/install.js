...
async function jitInstall(pkgName) {
    ...
    if (error) throw new Error(chalk`{bold {blue ${pkgName}} can not be resolved.}${EOL}`);
    if (deprecated) throw new Error(chalk`{bold {blue ${pkgName}} is deprecated. Refusing to install.}${EOL}`);
