...
function sourceExpander(config, filename) {
    // TODO: bury this back into Answers
    const source = config['source[+]'] || config.source;
    if (source == null) return config;
    return { ...config, source: expandPatterns(source, path.dirname(filename)) };
}
...
function load(options) {
    ...
    return paths.reduce(async (acc, file) => {
        ...
        if (Object.keys(readers).includes(ext)) acc[name] = LoadTask(await readers[ext](file));
        return acc;
    }, {});
}

function LoadTask(given) {
    if (given != null && [ 'string', 'function', 'object' ].includes(typeof given)) return given;
    throw new Error(`invalid Task: ${inspect(given)}`);
}
